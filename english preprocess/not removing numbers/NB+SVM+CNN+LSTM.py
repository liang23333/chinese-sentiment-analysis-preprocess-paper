...
import os
first_name="C:\\LAWHCA\\chinese-sentiment--analysis-preprocess\\txt_sentoken\\neg"
list_name=os.listdir("C:\\LAWHCA\\chinese-sentiment--analysis-preprocess\\txt_sentoken\\neg")
neg=[]
for i in range(1000):
    fname=first_name+"\\"+list_name[i]
    print(fname)
...
import os
first_name="C:\\LAWHCA\\chinese-sentiment--analysis-preprocess\\txt_sentoken\\pos"
list_name=os.listdir("C:\\LAWHCA\\chinese-sentiment--analysis-preprocess\\txt_sentoken\\pos")
pos=[]
for i in range(1000):
    fname=first_name+"\\"+list_name[i]
    print(fname)
...
def replace_english_stopwords(x):
    with open("C:\\LAWHCA\\chinese-sentiment--analysis-preprocess\\english_stopwords.txt","r",errors="ignore") as f:
        a=f.read()
        a=a.split()
        x=x.split()
        for i in a:
            while i in x:
                x.remove(i)
        return " ".join(x)
print(replace_english_stopwords("according  happy happy"))
...
for i in range(2000):
    ...
    data_all[i]=replace_english_stopwords(data_all[i])
    data_all[i]=remove_punctuation(data_all[i])
    #ata_all[i]=remove_number(data_all[i])
...
word_vectors = KeyedVectors.load_word2vec_format("C:\\Users\\Administrator\\Desktop\\GoogleNews-vectors-negative300.bin", binary=True)  # C binary forma


# In[11]:

print(len(word_vectors['woman']))
...
print(X_train.shape)
print(X_test.shape)
X_train = X_train.reshape(1500, 1,MAX, 300)
X_test = X_test.reshape(500, 1,MAX, 300)
